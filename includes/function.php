...
function checkSignSalt($data_info)
{
    ...
    if ($data_arr['package_name'] == PACKAGE_NAME || $data_arr['ios_bundle_identifier'] == IOS_BUNDLE_IDENTIFIER) {
        ...
        if ($data_arr['sign'] == '' && $data_arr['salt'] == '') {
            ...
            if(isset($data_arr['user_id']) && $data_arr['user_id']!=0){
        ...
        $set['HD_WALLPAPER'][] = array("success" => -1, "MSG" => "Invalid package name");
        header('Content-Type: application/json; charset=utf-8');
        echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
        exit();
    }
